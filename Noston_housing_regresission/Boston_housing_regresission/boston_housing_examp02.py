...
Original file is located at
    https://colab.research.google.com/drive/1o3MZO0NykLg8crIq0hXxJelVIdxLHvqp
"""
...
#   CRIM : 도시별 1인당 범죄율
#   ZN : 325,000제곱피트(sq.ft) 이상 부지로 지정된 주거용 토지의 비율
...
data_label =["범죄율","토지비율","비소매업면적","강인접","질소농도","평균박수","고택율","고용거리","고속접근","세율","교학생비율","흑인비율","하위계층비율"]
for ix,d in enumerate(x_train[:0]):
...
for ix in range(len(x_train[0])):
  if ix in except_datas:
    norm_mean.append(0)
    norm_std.append(1) # Use 1 for std if not scaling to avoid division by zero
    continue
  norm_mean.append(x_train[:,ix].mean())
  norm_std.append(x_train[:,ix].std())

def normal_data(target_data):
  normalized_data = np.copy(target_data) # Create a copy to avoid modifying original data
  for ix in range(len(normalized_data[0])):
    if ix in except_datas:
      continue
    # 데이터를 표준 정규분포로 스케일링
    normalized_data[:,ix] = (normalized_data[:,ix] - norm_mean[ix]) / norm_std[ix]
  return normalized_data

x_train = normal_data(x_train)
x_test = normal_data(x_test)

# Scale y_train and y_test using mean and std of y_train
y_mean = y_train.mean()
y_std = y_train.std()
...
# y_mean=y_train.mean()
# y_std=y_train.std()
# y_train= (y_train-mean)/y_std
# y_test = (y_test-mean)/y_std
fhist = model.fit(x_train,y_train_scaled,validation_data=(x_test,y_test_scaled),epochs=500)
# Scale y_train and y_test using mean and std of y_train
...
#print(y_pred[0]*y_std+y_mean)
#print(y_test[0]*y_std+y_mean)
